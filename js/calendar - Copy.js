...
// liturģiskā laika noteikšana
	if ( (gads[d].datums > cal.zsl1 && gads[d].datums < cal.gav0) || (gads[d].datums >= cal.pll2 && gads[d].datums <= cal.pll3) ) {
		...
		else if ( gads[d].datums > cal.epifany || nd == 0 ) {
			psn = 2;
			//nd = gads[d].datums.getDay();// nedēļas diena
		}
		else {
			psn = 1;
			nd = gads[d].datums.getDate();// datums
		}
		console.log("ZS psn:", psn, " nd:", nd);
		...
		if ( ( nd !=0 || ( nd !=6  ) ) // && stunda == 'vesperes'
			...
			&& gads[d].datums.getDay() !=0 ) {
				// sākot no 17.datuma			console.log('AHA', gads[d].datums.getDate()); //================================
				psn0=psn;
				psn = 'advents';
				nd = gads[d].datums.getDate(); // datums
				ned = gads[d].datums.getDay(); // nedēļas diena
				// Ja gads[d].datums.valueOf() >= new Date(year, 11, 24).valueOf() ir svētdiena, tad ņem 24.dec. liturģiju SK 2023.g.
...
// FESTUM === END
  ...
  if ( stunda == 'vesperes' && nd == 6) {  // console.log(liturgiskaisLaiks, 'Sestdienas vesperes')
	...
	console.log("ievadAnt. ",liturgiskaisLaiks, psn, nd);
	...
	const dienasPs = function(x){
		...
		if ( ( ll == 'zokt'  || ll == 'zsl' ) && liturgiskaisLaiks != 'festumShow' ) {
			...
			if (stunda == 'dienas' ) {
				...
				if ( ziemassvetki[psn][nd][stunda].ps1 == null ) {
	...
	const dienasAnt = function (x) {
		...
		if( stunda == 'dienas' && ( liturgiskaisLaiks == 'gavenis' || liturgiskaisLaiks == 'lieldienas' || (liturgiskaisLaiks == 'festumShow' && ll != 'pll') || typeof window[liturgiskaisLaiks][psn][nd][stunda]['stunda'][x]['ant'] == 'string') ) 	{ //PSN.ND.dienas.stunda[3].ant
			...
			if ( liturgiskaisLaiks != 'festumShow'  ) {
				if ( ( ll == 'zokt'  || ll == 'zsl' ) && ziemassvetki[psn][nd][stunda].ant1 == null ) { 
					console.log(d,' nu tad antifonas?', stunda, psn0, nd0, " vs ", psn, nd, ziemassvetki[psn][nd][stunda].ant1, ziemassvetki[psn][nd][stunda].ant1 != null );
...
//					ned = gads[d].datums.getDay(); // nedēļas diena
	...
	if ( stunda == 'laudes' || stunda == 'vesperes' || stunda == 'vesperesI' ) {
		...
		else if ( nd === 0 || ( nd === 6  && gads[d1].prior <= gads[d].prior ) && gads[d1].datums.valueOf() != cal.adv0.valueOf() ) { // svētdienu lūgšana 
						// gads[d1].datums.valueOf() != cal.adv0.valueOf() sestdienā pirms Adventa  nebija laudes lūgšanas

			liturgiskaisLaiks = liturgiskaisLaiks0;
			psn = psn0;
			if ( (liturgiskaisLaiks == 'advents' || liturgiskaisLaiks == 'gavenis' ) && nd === 6 && stunda == 'vesperes' ) {
				psn++; // adventā un gavēnī 1. vesperu lūgšana ir manīgajā, tāpēc jākoriģē
				
			} 
			console.log(liturgiskaisLaiks, 'psn:',psn);				
			nd =nd0;
			d = Number (d); // kaut kāda iemesla dēļ nomainās mainīgā tips
			if (nd === 6 && stunda == 'vesperes') {
...
/* 	if (liturgiskaisLaiks == 'festumShow' ) { // && gads[d].prior <= gads[d1].prior hrizostomam nerādīja
	...
	if ( stunda=='dienas' ) {
		if ( nd ==0 || (liturgiskaisLaiks == 'advents' || liturgiskaisLaiks == 'gavenis' || liturgiskaisLaiks == 'lieldienas' || liturgiskaisLaiks == 'ziemassvetki') ) { console.log( ' dienas lūgšana ' );
...
[11:50 pm, 04/12/2021] pr. Māris Laureckis: 1.himna 24.dec.-4.jan.
[11:52 pm, 04/12/2021] pr. Māris Laureckis: 2.himna no 5-Kunga kristīšana (šogad 9.jan.)
*/
	...
	else if(gads[d].litlaiks == 'adv' || gads[d].litlaiks == 'zsl' || gads[d].datums.valueOf() == cal.pll3.valueOf() ) antMarijai='maigaPestitajaMate';
