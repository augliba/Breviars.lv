...
var izlaistKompetoriju = "<br><span style='font-size: calc(var(--unit) * 0.03);text-transform: initial;'>Tie, kuri ņem dalību Lieldienu vigīlijas Sv. Misē, Kompletoriju var izlaist.</span>";
...
const Banner = function(x){ // liturģiskā krāsa un svētku nosaukums
	...
	if ( ( gads[d1].vesperesI == true || gads[d1].datums.getDay() == 0 ) && gads[d1].prior <= gads[d].prior && (stunda == 'vesperes' || stunda == 'vesperesI' || stunda == 'kompletorijs' ) ) {
		...
		if ( ( ll == 'zokt'  || ll == 'zsl' ) && liturgiskaisLaiks != 'festumShow' ) {
			...
			console.log( typeof ziemassvetki[psn][nd][stunda].stunda[dienasStunda].ps1 != "undefined" , ziemassvetki[psn][nd][stunda].stunda[dienasStunda].ps1 != "" )
				...
				if ( ziemassvetki[psn][nd][stunda].ps1 == "" ) { 
					document.querySelector('#psalms1').innerHTML = window[parastais[psn0][nd0][stunda].ps1];
					document.querySelector('#psalms2').innerHTML = window[parastais[psn0][nd0][stunda].ps2];
					document.querySelector('#psalms3').innerHTML = window[parastais[psn0][nd0][stunda].ps3];
				} 
				else {
					document.querySelector('#psalms1').innerHTML = window[ziemassvetki[psn][nd][stunda].ps1];
					document.querySelector('#psalms2').innerHTML = window[ziemassvetki[psn][nd][stunda].ps2];
		...
		if( stunda == 'dienas' 
			...
			// svarīgi ir salīdzinājuma secība     console.log(d,' nu tad antifonas?', stunda, psn0, nd0, " vs ", psn, nd, ziemassvetki[psn][nd][stunda].ant1, ziemassvetki[psn][nd][stunda].ant1 != null );
				...
				if (gads[d].datums.valueOf() == new Date(year, 11, 24).valueOf() && stunda == 'laudes' ) {
					...
					console.log(d,' haha ?', liturgiskaisLaiks, psn, nd, stunda);
					...
					if ( window[liturgiskaisLaiks][psn][nd][stunda].ant1 == '' ) {  // ja nav pirmās antifonas, tad visas ir no PLL
						//console.log("Šeit PLL antifonas", parastais, psn0, nd0, stunda);
							if (  ll == 'gav' ) psn0=psalmuNedela(psn); // gavēnī ir no 0. līdz 7. nedēļai, kas neatbilst PLL 4 nedēļu ciklam 
							sel = document.querySelectorAll('#ant1'); sel[0].innerHTML = sel[1].innerHTML = parastais[psn0][nd0][stunda].ant1;
							sel = document.querySelectorAll('#ant2'); sel[0].innerHTML = sel[1].innerHTML = parastais[psn0][nd0][stunda].ant2;
							sel = document.querySelectorAll('#ant3'); sel[0].innerHTML = sel[1].innerHTML = parastais[psn0][nd0][stunda].ant3;
						} 
...
console.log("read => adventa problēmas");
	...
	if(stunda != 'dienas') { // ja ir Laudes un Vesperes
		...
		for(i=0; i<diakoni.length; i++){ // diakoni = pieminasdienas.js
			if(gads[d].datums.valueOf() == new Date(year,diakoni[i].m-1,diakoni[i].d).valueOf()) sLugumi += "<li class='lugsana'>Mīlošais Tēvs, svētī diakonu "+diakoni[i].v+" un pārējos pastāvīgos diakonus, lai caur viņiem tiek pagodināts Tavs Dēls.<br><span class='bolditalic'>" + window[liturgiskaisLaiks][psn][nd][stunda].resplugums + "</span></li>";
		}
	...
	if ( stunda == 'laudes' || stunda == 'vesperes' || stunda == 'vesperesI' ) { 
		...
		if (liturgiskaisLaiks == 'festumShow'  && gads[d1].prior < gads[d].prior) { // && gads[d].prior < gads[d1].prior hrizostomam nerādīja
...
Banner('kompletorijs');
	...
	if ( gads[d].litlaiks == 'trd' || gads[d].litlaiks == 'lokt' || gads[d].litlaiks == 'zokt' ) { // gads[d].litlaiks == 'trd' || 
		...
		if (sd == 6 && gads[d].litlaiks == 'trd') komplet[nd].title += izlaistKompetoriju;
...
console.log("izvēlētais kompletorijs nd = ", nd);
...
//	var svetdiena = selectedSvetdiena;
	...
	if (gads[d].litlaiks == 'pll') liturgiskaislaiks = 'parastais';
	var nedela;
	var isFestum = (gads[d].liturgija) ? true : false;
	var naktsdiena = (laiks.getHours() < 9 || laiks.getHours() >= 21) ? 'nakts' : 'diena';
	console.log(liturgiskaislaiks,selectedNedela, psn,"=>",getPsalmWeek(psn),nd,nedela, isFestum, naktsdiena);
	...
	// containerHimna
	sel = document.querySelector('#himna');
	if ( naktsdiena == 'diena') sel.innerHTML = himnaOficiumDiena_1_0 ;
		else sel.innerHTML = himnaOficiumNakts_1_0;
	// containerPsalmodijs
	var psalmWeek = getPsalmWeek(psn);
  	for (i=1; i <= 3; i++) {
		var psName = 'ps'+i; 
		var antName = 'ant'+i;
		document.querySelector('#psalms'+ i ).innerHTML = window[officium.psalmodijs[getPsalmWeek(psn)][nd][psName]];
		sel = document.querySelectorAll('#ant' +i)
		sel[0].innerHTML = sel[1].innerHTML = officium.psalmodijs[getPsalmWeek(psn)][nd][antName];
	}
	document.querySelector('#responsorijsPs').innerHTML = '<p>\
	<span class="redbold">℣. </span>' + officium.psalmodijs[getPsalmWeek(psn)][nd].atb1 + '<br>\
	<span class="redbold">℟. </span>' + officium.psalmodijs[getPsalmWeek(psn)][nd].atb2 + '</p>';
	// LASĪJUMi
	for (i=1; i <= 2; i++) {
		var pirmaisotrais = (i ==1) ? 'I': 'II';
		document.querySelector('#containerLasijums' + i ).innerHTML = '<hr><h4><span class="redbold">'+pirmaisotrais+' LASĪJUMS</span></h4><p><span class="redbold">' + officium.parastais[psn][nd][i].lasijumstitle + '</span></p><p>' + officium.parastais[psn][nd][i].lasijumstext + '</p></div>';
		sel = document.querySelector('#responsorijs' + i ).innerHTML = '<p>\
			<span class="redbold">℣. </span>' + officium.parastais[psn][nd][i].vers1 + '<br>\
			<span class="redbold">℟. </span>' + officium.parastais[psn][nd][i].resp + '<br>\
			<span class="redbold">℣. </span>' + officium.parastais[psn][nd][i].vers2 + '<br>\
			<span class="redbold">℟. </span>' + officium.parastais[psn][nd][i].resp + '</p>';
	}

	// noslēguma Lūgšana
	document.querySelector("#noslegumaLugsana").innerHTML = '<p><span class="redbold">℣. </span>Lūgsimies.</p><p>'+lugsana_pll_sv[psn]+'</p><p><span class="redbold">℟. </span>Āmen.</p>';
